...
The `fm-odata-js` library allows you to trigger scripts from three different entry points. The choice of entry point determines the path suffix used in the `POST` request and how FileMaker Server initializes the script's environment [src/scripts.ts:4-9]().
...
### Error Hierarchy
`FMScriptError` extends `FMODataError`, ensuring that generic error handling catches script failures while allowing specific logic for FileMaker error codes [src/errors.ts:85-111]().
