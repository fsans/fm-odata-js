...
This page documents the testing infrastructure for `fm-odata-js`, which utilizes **Vitest** for unit and integration testing and **Playwright** for end-to-end (e2e) verification. The suite is designed to be fast and offline-first by default, with an opt-in mechanism for live testing against real FileMaker Server (FMS) instances.
...
The unit tests verify the internal logic of the library without network side effects. These tests use `vi.fn()` to mock the `fetch` implementation provided to the `FMOData` constructor [tests/unit/scripts.test.ts:16-27]().
...
**Sources:** [tests/unit/url.test.ts:11-113](), [tests/unit/query.test.ts:45-175](), [tests/unit/scripts.test.ts:29-126](), [tests/unit/errors.test.ts:23-62]()
...
4.  **Delete**: Removes the record and verifies that a subsequent `GET` returns a `404` via `FMODataError` [tests/integration/live.test.ts:77-89]().
