...
The `examples/consumer-node` project provides a minimal, functional demonstration of how to integrate `fm-odata-js` into a standalone Node.js environment. It illustrates the standard pattern for library consumption, query building, script execution, and robust error handling [examples/consumer-node/README.md:3-8]().
...
The example instantiates the `FMOData` client and uses the fluent `Query` builder to fetch data from the `Contacts.fmp12` solution [examples/consumer-node/index.mjs:50-55]().
...
2.  **`FMODataError`**: Catches general OData or HTTP errors (e.g., 401 Unauthorized, 404 Not Found) and logs the status and message [examples/consumer-node/index.mjs:90-91]().
...
graph TD
    subgraph "index.mjs"
        ...
        CATCH_ODATA{"err instanceof FMODataError?"}
        LOG_SKIP["Log: Skipping (Script Missing)"]
        LOG_ERR["Log: HTTP/OData Error"]
    end

    subgraph "fm-odata-js internals"
