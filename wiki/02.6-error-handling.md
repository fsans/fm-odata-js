...
### FMODataError
The `FMODataError` is the base class for all errors thrown during HTTP operations [src/errors.ts:4-26](). It captures the essential context of the failed request, including the HTTP status and the raw error payload.
...
### FMScriptError
The `FMScriptError` is a specialized subclass of `FMODataError` [src/errors.ts:85-111](). It is thrown when an OData Action (script execution) completes with a non-zero FileMaker script error. 
...
classDiagram
    ...
    Error <|-- FMODataError
    FMODataError <|-- FMScriptError

    style FMODataError stroke-width:2px
...
FileMaker Server's OData implementation may return errors in different formats depending on the nature of the failure. The `parseErrorResponse()` function is an internal utility that normalizes these disparate formats into a single `FMODataError` instance [src/errors.ts:35-75]().
...
| :--- | :--- | :--- |
| `FMODataError` | Base class for HTTP errors and data validation failures. | [src/errors.ts:4-26]() |
