...
The build process is designed to produce high-performance, tree-shakable ESM modules with zero runtime dependencies. It consists of three primary stages executed via the `npm run build` command [package.json:24-24]().
...
graph TD
  ...
  ESB -- "--minify" --> MIN
```
**Sources:** [package.json:24-27](), [tsconfig.build.json:1-10]()
...
| **Minified ESM** | `dist/fm-odata.esm.min.js` | Optimized version for Web Viewers to reduce memory overhead [package.json:27-27](). |
...
**Sources:** [package.json:6-14](), [package.json:26-27]()
