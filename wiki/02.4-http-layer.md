...
The library supports both **Basic** (Username/Password) and **Bearer** (Token-based) authentication. Because FileMaker Server's OData implementation primarily relies on HTTP Basic auth, the layer provides helpers to ensure correct encoding and scheme detection.
...
Sources: [src/http.ts:42-61](), [src/http.ts:72-78]()
